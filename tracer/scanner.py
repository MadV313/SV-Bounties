...
async def scan_adm_line(guild_id: int, line: str, source_ref: str, ts: datetime):
    ...
    try:
        ...
        if not m:
            logger.debug(f"[Guild {guild_id}] Ignored ADM line: {line}")
            return
        ...
        logger.info(f"[Guild {guild_id}] Tracked {gamertag} at ({x},{z}) from {source_ref}")
    except Exception as e:
        logger.error(f"[Guild {guild_id}] Error parsing ADM line: {line} ({e})", exc_info=True)
